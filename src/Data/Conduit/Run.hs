...
runTransparent (OutputSink otr) exe args = fatal $
    tryProcess Nothing (transDump stdout .| otr) (transDump stderr .| otr) exe args
        >>= either throwIO pure
...
runSemiTransparent
    ...
    -> [String]
    -> IO (StdOut out, StdErr out)
runSemiTransparent (OutputSink otr) exe args = fatal $
    tryProcess Nothing otr (transDump stderr .| otr) exe args
        >>= either throwIO pure
...
runOpaque
    ...
    -> [String]
    -> IO (StdOut out, StdErr out)
runOpaque (OutputSink otr) exe args = fatal $
    tryProcess Nothing otr otr exe args
        >>= either throwIO pure
...
tryProcess mInC outC errC exe args = do
